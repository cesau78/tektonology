// --- TEKTONOLOGY BOLT DRYING RACK CONFIG ---
// Shared parameters for drying rack plates and legs.

tolerance     = 0.15;    // general clearance per side (mm)

// --- Bolt Profile: M3x20 Socket Head Cap Screw ---
bolt_dia        = 3.0;    // shaft diameter (mm)
bolt_length     = 20;     // shaft length under head (mm)
bolt_clearance  = 0.3;    // radial clearance for bolt holes (mm)
head_dia        = 6.0;    // socket head cap diameter (mm)
head_height     = 3.5;    // socket head height (mm)
...
bottom_plate_z  = clearance_mm + compound_length ; // bottom plate Z (backed off by tolerance)
top_plate_z     = bottom_plate_z + plate_thick + plate_gap; // top plate Z (backed off by tolerance)
...
slot_h        = plate_thick + tolerance * 2;   // slot height (Z)
slot_y        = plate_y + tolerance * 2;       // slot width (Y) — plate fits inside
