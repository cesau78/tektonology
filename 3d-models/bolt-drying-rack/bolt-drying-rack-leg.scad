...
module channel_slot(z_base) {
    translate([-0.1, channel_wall - tolerance, z_base - tolerance])
        cube([leg_post_x + 0.2, slot_y, slot_h]);
}

// Ceiling-only detent: plate rests on slot floor with full clearance,
// ceiling detent presses down to retain. Running clearance = tolerance * 2.
module snap_detent(z_base) {
    translate([leg_post_x / 2 - detent_w / 2, channel_wall - tolerance, z_base + slot_h - 0.01 + tolerance])
        hull() {
...
module post_holes() {
    slot_bottom_lo = bottom_plate_z - tolerance;
    slot_bottom_hi = bottom_plate_z + slot_h + tolerance;
    slot_top_lo    = top_plate_z - tolerance;
    slot_top_hi    = top_plate_z + slot_h + tolerance;
